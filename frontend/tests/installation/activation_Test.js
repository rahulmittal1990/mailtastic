...
describe('mailtasticApp.installation', function() {
    ...
    describe('ActivationController controller', function() {

        it("should mode is equal to employee....", function() {
            $scope = {};
            $scope.$watch = {};
            spyOn($scope, '$watch')
            spyOn(bootbox, 'alert')
            var controller = $controller('ActivationCtrl', {
                $scope: $scope,
                signatureHelperService: sigService

            });

            expect($scope.mode).toEqual("employee");
