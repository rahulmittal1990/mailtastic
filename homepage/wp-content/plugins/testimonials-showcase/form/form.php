...
function ttshowcase_build_form($atts) {
	...
	$tt_fields_order = cmshowcase_get_option('order', $section, 'name,subtitle,url,image,title,testimonial,rating,email,yesOrNo,humanVerification');
	...
	if(isset($_POST['tt_submitted']) && isset($_POST['post_nonce_field']) && wp_verify_nonce($_POST['post_nonce_field'], 'post_nonce')) {
		...
		if(in_array('title',$mandatory) && isset($_POST['_aditional_info_review_title']) && trim($_POST['_aditional_info_review_title']) === '') {
				...
				if($boolean_field == 'on') {
					...
					$yesno_form_html .= '<fieldset>
						...
						foreach ($bool_opt as $key => $value) {
								...
								$yesno_form_html .= '<option value="'.$key.'" '. selected($tt_curr_selected, $key , false).' >'.$value.'</option>';
